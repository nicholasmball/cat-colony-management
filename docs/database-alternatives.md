...
The app is built fairly deeply on Supabase: Postgres **row-level security** (our multi-tenant safety net), **database functions/triggers**, and **Supabase Auth** (login, invites). Those are real value, not boilerplate — so "swap the database" ranges from *easy* (a Postgres-compatible host) to *a rewrite* (a totally different kind of database). The table below rates that honestly.
